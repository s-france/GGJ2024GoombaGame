...
func jump():
	...
	if is_on_floor() and !jumpBufferTimer.is_stopped():
		...
		if Input.is_action_just_released("jump") and velocity.y < JUMPSPEED/2.0:
			velocity.y = JUMPSPEED/2.0
			
		if Input.is_action_just_pressed("jump") and double_jump_available and not just_wall_jumped:
